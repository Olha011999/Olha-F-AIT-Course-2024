...
class ToDoListImplTest {
    ...
    @BeforeEach
    void setUp() {//void не тестиреются assert
    ...
    void addTask() {
        ...
        assertFalse(toDoList.addTask(new Task("Do homework", now.minusDays(2))));// duplicate task
    ...
    void removeTask() {
        //ToDo
        Task removedTask = toDoList.removeTask(1);
        assertNotNull(removedTask);
        assertEquals("Do homework", removedTask.getTask());
...
//        toDoList.viewTasks();
        ...
        assertEquals("Do homework", allTasks.get(0).getTask());
    }
