package homework_39.ait.supermarket_hashset.test;

import homework_39.ait.supermarket_hashset.dao.Supermarket;
import homework_39.ait.supermarket_hashset.dao.SupermarketImpl;
import homework_39.ait.supermarket_hashset.model.Product;
...
class SupermarketImplTest {
    ...
    void addProduct() {
        ...
        assertFalse(supermarket.addProduct(products[3]));//выдыет фальш , потомучто такой элемент уже существует
    ...
    void findByCategory() {
        ...
        Set<Product> actual = (Set<Product>) supermarket.findByCategory("Bread"); // безопасный кастинг
        int l = actual.size();
    ...
    void findProductsWithExpiredDate() {
        ...
        Iterable<Product> expected = Arrays.asList(products[4], products[1]);// в хешсет поменяли местами продукты//просроченные
        assertIterableEquals(actual, expected);
    }
